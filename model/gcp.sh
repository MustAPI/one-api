...
echo "        location /az/ {" >> ${CONF_NGINX}
echo "            if (\$request_uri ~* \"^/az/(https?://[^/]+)(/?.*)\") {" >> ${CONF_NGINX}
echo "                set \$target_host \$1;" >> ${CONF_NGINX}
echo "                set \$target_path \$2;" >> ${CONF_NGINX}
echo "                proxy_pass \$target_host\$target_path;" >> ${CONF_NGINX}
echo "                proxy_ssl_server_name on;" >> ${CONF_NGINX}
echo "                proxy_buffering off;" >> ${CONF_NGINX}
echo "                proxy_set_header Host \$target_host;" >> ${CONF_NGINX}
echo "                proxy_set_header X-Real-IP \$remote_addr;" >> ${CONF_NGINX}
echo "                proxy_set_header X-Forwarded-For \$proxy_add_x_forwarded_for;" >> ${CONF_NGINX}
echo "                proxy_set_header X-Forwarded-Proto \$scheme;" >> ${CONF_NGINX}
...
echo "                break;" >> ${CONF_NGINX}
echo "            }" >> ${CONF_NGINX}
echo "            return 404;" >> ${CONF_NGINX}
echo "        }" >> ${CONF_NGINX}
